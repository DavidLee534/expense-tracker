...
- [ ] `signup.html` / `signup.js` — 이름·이메일·비밀번호 검증, 가입 즉시 자동 로그인
- [ ] `login.html` / `login.js` — 이메일/비밀번호 로그인, `?next=` 파라미터로 로그인 후 원래 위치 복귀, 정지 계정 에러 메시지
- [ ] 이미 로그인 상태면 두 페이지 모두 자동으로 홈(`/index.html`)으로 리다이렉트
...
- [ ] `index.html` / `index.css` / `index.js`
  - [ ] 월 선택기 + 연 선택기
  - [ ] 카테고리별 비중 차트 — 도넛/막대 전환 토글 버튼 (도넛: CSS `conic-gradient` 기반, 막대: 카테고리별 가로 막대), 선택 상태는 로컬 상태로 유지
  - [ ] 월별 지출 추이 막대 차트
  - [ ] 카테고리 랭킹 리스트 (금액/비율)
...
- [ ] `index.html` / `index.css` / `index.js`
  - [ ] 상단 네비게이션 (대시보드/지출내역/통계 + 로그아웃)
  - [ ] 이번 달 총지출 요약 카드, 전월 대비 증감
  - [ ] 카테고리별 비중 미니 차트
  - [ ] 최근 지출 내역 5건
...
- [ ] `list.html` / `list.css` / `list.js`
  - [ ] 회원 목록 테이블(이름·이메일·가입일·상태), 검색
  - [ ] 정지/해제 토글, 상세 페이지 링크
- [ ] `detail.html` / `detail.css` / `detail.js`
  - [ ] 회원 기본 정보 + 해당 회원의 지출 요약(총액, 최근 내역)
  - [ ] 정지/해제, 계정 삭제(소유 지출 내역 함께 삭제)
...
- [ ] `index.html` / `index.css` / `index.js`
  - [ ] 사이드바 (회원 관리 / 대시보드 네비게이션 + 로그아웃)
  - [ ] 통계 카드: 전체 회원수, 총 지출액, 이번 달 지출액
  - [ ] 인기 카테고리 TOP5 차트
  - [ ] 최근 가입 회원 리스트
